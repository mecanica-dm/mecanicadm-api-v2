...
class UpdateClientServiceTest {
    ...
    void shouldUpdateClientSuccessfully() {
        ...
        when(repository.existsClientByDocumentAndIdNot(command.document(), clientId)).thenReturn(false);
        when(repository.existsClientByEmailAndIdNot(command.email(), clientId)).thenReturn(false);
    ...
    void shouldUpdateSuccessfullyWhenDocumentAndEmailAreSame() {
        ...
        verify(repository).existsClientByEmailAndIdNot(command.email(), clientId);
        verify(repository).existsClientByDocumentAndIdNot(command.document(), clientId);
    }
    ...
    void shouldThrowExceptionWhenEmailExistsForOtherClient() {
        when(repository.findById(clientId)).thenReturn(Optional.of(existingClient));
        when(existingClient.getId()).thenReturn(clientId);
        when(repository.existsClientByEmailAndIdNot(command.email(), clientId)).thenReturn(true);
    ...
    void shouldThrowExceptionWhenDocumentExistsForOtherClient() {
        when(repository.findById(clientId)).thenReturn(Optional.of(existingClient));
        when(existingClient.getId()).thenReturn(clientId);
        when(repository.existsClientByDocumentAndIdNot(command.document(), clientId)).thenReturn(true);
