...
public class AuthenticateUserUseCase implements UseCase<AuthenticateUserQuery, UserAuthentication> {
    ...
    @Override
    public UserAuthentication execute(AuthenticateUserQuery query) {
        ...
        return new UserAuthentication(token, user.getId(), user.getName(), user.getEmail());
    }
}
